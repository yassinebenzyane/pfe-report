...
AgriConnect relies primarily on WhatsApp as the main communication channel, while a web application enables agronomists and administrators to manage cases and monitor interventions. More specifically, the platform is structured around a central orchestrator responsible for activating several specialized agents according to the nature of each interaction. These agents include a vision agent for crop image analysis, a retrieval-augmented generation agent for querying the agricultural knowledge base, a weather agent for contextual environmental reasoning, and an escalation agent for urgent or uncertain cases. The orchestrator itself manages conversational continuity and farmer-level personalization by integrating interaction history and farmer profile data at each reasoning step.
...
\section{Scientific and Technical Contribution}
...
The remainder of this report is organized as follows. The first chapter reviews the state of the art in digital agricultural advisory systems and translates the identified gaps into a structured requirements specification. The second chapter presents the system architecture and design choices. The third chapter describes the implementation of the platform. The fourth chapter evaluates the system through scenario-based testing and functional validation, and discusses its limitations and future perspectives.
